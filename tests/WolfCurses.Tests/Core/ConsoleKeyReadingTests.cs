...
namespace WolfCurses.Tests.Core
{
    ...
    public class ConsoleKeyReadingTests
    {
        ...
        [Fact]
        public void SendConsoleKey_WithNoWindowOrFormAttached_RoutesEverythingNowhere()
        {
            // The rule in one place: input with nobody attached to hear it goes nowhere and leaves nothing behind.
            // Characters never enter the buffer (nothing is accepting input), keys are spent-and-dropped rather than
            // saved, and ENTER submits only emptiness — so a window and form attached a moment later start clean,
            // hearing nothing that was pressed before they existed.
            var app = new TestSimulationApp();
            app.InputManager.SendConsoleKey(Key('h', ConsoleKey.H));
            app.InputManager.SendConsoleKey(Key('i', ConsoleKey.I));
            app.InputManager.SendConsoleKey(Key('\r', ConsoleKey.Enter));
            app.InputManager.SendConsoleKey(Key('\0', ConsoleKey.LeftArrow));
