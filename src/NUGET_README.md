...
**Clickable layouts.** `Keypad` is a grid of labelled, clickable cells with spanning keys, whose layout is read by both the drawing and the hit test so the two cannot disagree. `MonthGrid` is the same idea for a month: six week rows always, today and a chosen day picked out, and today *told* rather than read from the clock so it can be tested and so a screen left open past midnight moves its own highlight.
...
- **[The office suite](https://github.com/Maxwolf/WolfCurses/blob/master/example/WolfCurses.Apps/README.md)**: a word processor after the MS-DOS Editor, a BASIC environment after the one that shipped with DOS, a spreadsheet, a desk calculator, and a calendar.
